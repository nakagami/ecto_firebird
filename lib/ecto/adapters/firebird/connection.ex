defmodule Ecto.Adapters.Firebird.Connection do
  ...
  import Ecto.Adapters.Firebird.DataType
  ...
  def start_link(opts) do
    ...
    DBConnection.start_link(Firebirdex.Connection, opts)
  end
  ...
  def child_spec(options) do
    ...
    DBConnection.child_spec(Firebirdex.Connection, options)
  end
  ...
  @impl true
  def execute(conn, %Firebirdex.Query{stmt: stmt} = cached, params, options)
      when stmt != nil do
  ...
  def execute(
        conn,
        %Firebirdex.Query{statement: statement, stmt: nil},
        params,
        options
      ) do
  ...
  def execute(conn, query, params, options) do
    case DBConnection.execute(conn, query, params, options) do
      ...
      {:error, %Firebirdex.Error{}} = error -> error
      {:error, err} -> raise err
    end
  end
  ...
  def query(conn, sql, params, options) do
    Firebirdex.query(conn, sql, params, options)
  end

  @impl true
  def query_many(_conn, _sql, _params, _opts) do
    raise RuntimeError, "query_many is not supported in the Firebird adapter"
  end
  ...
  def all(%Ecto.Query{lock: lock}) when lock != nil do
    raise ArgumentError, "locks are not supported by Firebird"
  end
  ...
  def execute_ddl({_command, %Table{options: options}, _}) when is_list(options) do
    raise ArgumentError, "Firebird adapter does not support keyword lists in :options"
  end
  ...
  def execute_ddl({:create_if_not_exists, %Table{} = table, columns}) do
    {table, composite_pk_def} = composite_pk_definition(table, columns)
    composite_fk_defs = composite_fk_definitions(table, columns)
    # I couldn't implement "if not exists" well, so I used "recreate table".
    [
      [
        "CREATE TABLE ",
        quote_table(table.prefix, table.name),
  ...
  def execute_ddl({_, %Index{concurrently: true}}) do
    raise ArgumentError, "`concurrently` is not supported with Firebird"
  end

  @impl true
  def execute_ddl({_, %Index{only: true}}) do
    raise ArgumentError, "`only` is not supported with Firebird"
  end

  @impl true
  def execute_ddl({_, %Index{include: x}}) when length(x) != 0 do
    raise ArgumentError, "`include` is not supported with Firebird"
  end

  @impl true
  def execute_ddl({_, %Index{using: x}}) when not is_nil(x) do
    raise ArgumentError, "`using` is not supported with Firebird"
  end

  @impl true
  def execute_ddl({_, %Index{nulls_distinct: x}}) when not is_nil(x) do
    raise ArgumentError, "`nulls_distinct` is not supported with Firebird"
  end
  ...
  def execute_ddl(keyword) when is_list(keyword) do
    raise ArgumentError, "Firebird adapter does not support keyword lists in execute"
  end
  ...
  def execute_ddl({:create, %Constraint{}}) do
    raise ArgumentError, "Firebird does not support ALTER TABLE ADD CONSTRAINT."
  end
  ...
  def execute_ddl({:drop, %Constraint{}, _mode}) do
    raise ArgumentError, "Firebird does not support ALTER TABLE DROP CONSTRAINT."
  end
  ...
  defp on_conflict({:replace_all, _, {:constraint, _}}, _header) do
    raise ArgumentError, "Upsert in Firebird does not support ON CONSTRAINT"
  end

  defp on_conflict({:replace_all, _, []}, _header) do
    raise ArgumentError, "Upsert in Firebird requires :conflict_target"
  end
  ...
  def insert_all(%Ecto.Query{wheres: []} = _query, on_conflict, _counter)
    ...
    raise ArgumentError,
          "Firebird requires a where clause to avoid ambiguity. Even simply specify where: true will work"
  end
        ...
        case elem(sources, idx) do
          {source, _, nil} ->
            raise Ecto.QueryError,
              query: query,
              message: """
              Firebird does not support selecting all fields from #{source} \
  ...
  defp update_op(:push, _quoted_key, _value, _sources, query) do
    raise Ecto.QueryError,
      query: query,
      message: "Arrays are not supported for Firebird"
  end
  ...
  defp update_op(command, _quoted_key, _value, _sources, query) do
    raise Ecto.QueryError,
      query: query,
      message: "Unknown update operation #{inspect(command)} for Firebird"
  end
    ...
    Enum.map(joins, fn
      %JoinExpr{
        ...
        if hints != [] do
          raise Ecto.QueryError,
            query: query,
            message: "join hints are not supported by Firebird"
        end
  ...
  defp join_qual(mode, query) do
    raise Ecto.QueryError,
      query: query,
      message: "join `#{inspect(mode)}` not supported by Firebird"
  end
  ...
  defp order_by_expr({dir, expression}, sources, query) do
    ...
    case dir do
      ...
      _ ->
        raise Ecto.QueryError,
          query: query,
          message: "#{dir} is not supported in ORDER BY in Firebird"
    end
  end
  ...
  defp combination({:except_all, query}) do
    raise Ecto.QueryError,
      query: query,
      message: "Firebird does not support EXCEPT ALL"
  end

  defp combination({:intersect_all, query}) do
    raise Ecto.QueryError,
      query: query,
      message: "Firebird does not INTERSECT ALL"
  end

  def lock(query, _sources) do
    raise Ecto.QueryError,
      query: query,
      message: "Firebird does not support locks"
  end
  ...
  # workaround for the fact that Firebird not support specifying table
  # in the returning clause. when a later release adds the ability, this code can be deleted
  ...
  def expr({:fragment, _, [kw]}, _sources, query)
    ...
    raise Ecto.QueryError,
      query: query,
      message: "Firebird adapter does not support keyword or interpolated fragments"
  end
  ...
  def expr({:ilike, _, [_, _]}, _sources, query) do
    raise Ecto.QueryError,
      query: query,
      message: "ilike is not supported by Firebird"
  end
  ...
  def expr(list, _sources, query) when is_list(list) do
    raise Ecto.QueryError,
      query: query,
      message: "Array literals are not supported by Firebird"
  end
  ...
  defp column_change(_table, {:modify, _name, _type, _opts}) do
    raise ArgumentError, "ALTER COLUMN not supported by Firebird"
  end
  ...
  defp column_change(_table, _) do
    raise ArgumentError, "Not supported by Firebird"
  end
  ...
  defp pk_expr(true, type) when type in [:serial, :bigserial],
    do: " GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY"
  ...
  defp quote_table(_prefix, _name) do
    raise ArgumentError, "Firebird does not support table prefixes"
  end
