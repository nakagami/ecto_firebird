defmodule Ecto.Adapters.Firebird.ConnectionTest do
  use ExUnit.Case, async: true

  import Ecto.Query

  alias Ecto.Adapters.Firebird.Connection, as: SQL
  ...
  defmodule Schema do
    ...
    schema "schema" do
      ...
      has_many(:comments, Ecto.Adapters.Firebird.ConnectionTest.Schema2,
        references: :x,
        foreign_key: :z
      )

      has_one(:permalink, Ecto.Adapters.Firebird.ConnectionTest.Schema3,
        references: :y,
        foreign_key: :id
      )
    end
  ...
  defmodule Schema2 do
    ...
    schema "schema2" do
      belongs_to(:post, Ecto.Adapters.Firebird.ConnectionTest.Schema,
        references: :x,
        foreign_key: :z
      )
    end
  ...
  test "from with hints" do
    query = Schema |> from(hints: "INDEXED BY FOO") |> select([r], r.x) |> plan()
    assert all(query) == ~s{SELECT s0."x" FROM "schema" AS s0 INDEXED BY FOO}
  end
  ...
  test "from without schema" do
    ...
    assert_raise Ecto.QueryError,
                 ~r"Firebird does not support selecting all fields from \"posts\" without a schema",
                 fn ->
  ...
  test "from with fragment" do
    ...
    assert_raise Ecto.QueryError, ~r/^Firebird does not support/, fn ->
  ...
  test "parent binding subquery and CTE" do
    ...
    assert_raise Ecto.QueryError,
                 ~r/join `:left_lateral` not supported by Firebird/,
                 fn ->
                   all(query)
                 end
  end
  ...
  test "aggregate filters" do
    ...
    assert all(query) ==
             ~s{SELECT count(*) FILTER (WHERE s0."x" > 10) FROM "schema" AS s0}
  end
  ...
  test "distinct" do
    ...
    assert_raise Ecto.QueryError,
                 ~r"DISTINCT with multiple columns is not supported by Firebird",
                 fn ->
                   query =
                     Schema
                     |> distinct([r], [r.x, r.y])
  ...
  test "where" do
    ...
    assert all(query) ==
             ~s{SELECT s0."x" FROM "schema" AS s0 WHERE ((s0."x",s0."y") > (1,2))}
  end
  ...
  test "lock" do
    assert_raise ArgumentError, "locks are not supported by Firebird", fn ->
      Schema |> lock("FOR SHARE NOWAIT") |> select([], true) |> plan() |> all()
    end
  ...
  test "aliasing a selected value with selected_as/2" do
    ...
    assert all(query) ==
             ~s{SELECT sum(coalesce(s0."x", 0)) AS "integer" FROM "schema" AS s0}
  end
  ...
  test "group_by can reference the alias of a selected value with selected_as/1" do
    ...
    assert all(query) ==
             ~s{SELECT s0."x" AS "integer" FROM "schema" AS s0 GROUP BY "integer"}
  end
  ...
  test "order_by can reference the alias of a selected value with selected_as/1" do
    ...
    assert all(query) ==
             ~s{SELECT s0."x" AS "integer" FROM "schema" AS s0 ORDER BY "integer"}
  ...
  test "datetime_add" do
    ...
    assert all(query) ==
             ~s{SELECT 1 FROM "schema" AS s0 WHERE (DATEADD (month,1,s0."foo") > s0."bar")}
  end

  test "tagged type" do
    query = Schema |> select([t], type(t.x + t.y, :integer)) |> plan()
    assert all(query) == ~s{SELECT CAST(s0."x" + s0."y" AS INTEGER) FROM "schema" AS s0}
    ...
    assert all(query) ==
             ~s{SELECT CAST(? AS CHAR(36)) FROM "schema" AS s0}
    ...
    assert all(query) == ~s{SELECT CAST(? AS BLOB SUB_TYPE 1) FROM "schema" AS s0}
  end
  ...
  test "json_extract_path" do
    ...
    assert all(query) ==
             ~s|SELECT json_extract(s0.\"meta\", '$[0][1]') FROM "schema" AS s0|

    query = Schema |> select([s], json_extract_path(s.meta, ["a", "b"])) |> plan()

    assert all(query) ==
             ~s|SELECT json_extract(s0.\"meta\", '$.a.b') FROM "schema" AS s0|

    query = Schema |> select([s], json_extract_path(s.meta, ["'a"])) |> plan()

    assert all(query) ==
             ~s|SELECT json_extract(s0.\"meta\", '$.''a') FROM "schema" AS s0|
  ...
  test "optimized json_extract_path" do
    ...
    assert all(query) ==
             ~s|SELECT 1 FROM "schema" AS s0 WHERE (json_extract(s0.\"meta\", '$.id') = 123)|
    ...
    query =
      Schema |> where([s], s.meta["tags"][0]["name"] == "123") |> select(true) |> plan()

    assert all(query) ==
             ~s|SELECT 1 FROM "schema" AS s0 WHERE (json_extract(s0."meta", '$.tags[0].name') = '123')|

    query = Schema |> where([s], s.meta[0] == "123") |> select(true) |> plan()

    assert all(query) ==
             ~s|SELECT 1 FROM "schema" AS s0 WHERE (json_extract(s0.\"meta\", '$[0]') = '123')|

    query = Schema |> where([s], s.meta["enabled"] == true) |> select(true) |> plan()

    assert all(query) ==
             ~s|SELECT 1 FROM "schema" AS s0 WHERE (json_extract(s0."meta", '$.enabled') = 1)|
    ...
    assert all(query) ==
             ~s|SELECT 1 FROM "schema" AS s0 WHERE (json_extract(s0."meta", '$.extra[0].enabled') = 0)|
  end
  ...
  test "order_by and types" do
    ...
    assert all(query) ==
             "SELECT 1 FROM \"schema3\" AS s0 ORDER BY CAST(s0.\"binary\" AS REAL)"
  end
  ...
  test "fragments and types" do
    ...
    result =
      "SELECT 1 FROM \"schema\" AS s0 " <>
        "WHERE (extract(? from s0.\"start_time\") = CAST(? AS INTEGER)) " <>
        "AND (extract(? from s0.\"start_time\") = CAST(? AS INTEGER))"
  ...
  test "build_explain_query" do
    assert SQL.build_explain_query("SELECT 1", :query_plan) ==
             "EXPLAIN QUERY PLAN SELECT 1"

    assert SQL.build_explain_query("SELECT 1", :instructions) == "EXPLAIN SELECT 1"
  end
  ...
  test "update all with returning" do
    query =
      from(m in Schema, update: [set: [x: 0]]) |> select([m], m) |> plan(:update_all)

    assert update_all(query) ==
             ~s{UPDATE "schema" AS s0 SET "x" = 0 RETURNING "id", "x", "y", "z", "w", "meta"}
  ...
  test "update all with subquery" do
    ...
    {planned_query, cast_params, dump_params} =
      Ecto.Adapter.Queryable.plan_query(:update_all, Ecto.Adapters.Firebird, query)
  ...
  test "update all with prefix" do
    ...
    assert_raise ArgumentError, "Firebird does not support table prefixes", fn ->
      update_all(query)
    end
  end
  ...
  test "delete all" do
    ...
    query =
      from(e in Schema, where: e.x == 123, join: q in Schema2, on: e.x == q.z) |> plan()
  ...
  test "delete all with returning" do
    ...
    assert delete_all(query) ==
             ~s{DELETE FROM "schema" AS s0 RETURNING "id", "x", "y", "z", "w", "meta"}
  end
  ...
  describe "windows and partitions" do
    ...
    test "custom aggregation over all" do
      ...
      assert all(query) ==
               ~s{SELECT custom_function(s0."x") OVER () FROM "schema" AS s0}
    end
  ...
  test "join with hints" do
    assert_raise Ecto.QueryError, ~r/join hints are not supported by Firebird/, fn ->
      Schema
  ...
  test "cross join with fragment" do
    ...
    assert all(query) ==
             ~s{SELECT s0."id" FROM "schema" AS s0 CROSS JOIN json_each(s0."j") AS f1}
  end
  ...
  test "association join belongs_to" do
    query =
      Schema2 |> join(:inner, [c], p in assoc(c, :post)) |> select([], true) |> plan()
  ...
  test "insert" do
    ...
    assert_raise(
      ArgumentError,
      "Cell-wise default values are not supported on INSERT statements by Firebird",
      fn ->
        insert(nil, "schema", [:x, :y], [[:x, :y], [nil, :z]], {:raise, [], []}, [:id])
      end
    )
  ...
  test "insert with on conflict" do
    ...
    assert_raise ArgumentError, "Upsert in Firebird requires :conflict_target", fn ->
      conflict_target = []
    ...
    assert_raise ArgumentError,
                 "Upsert in Firebird does not support ON CONSTRAINT",
                 fn ->
                   insert(
                     nil,
                     "schema",
  ...
  test "insert with query as rows" do
    ...
    assert_raise ArgumentError, fn ->
      insert(nil, "schema", [:foo, :bar], query, {:raise, [], []}, [:foo])
    end
  ...
  test "create table" do
    ...
    assert execute_ddl(create) == [
             ...
             "tags" BLOB SUB_TYPE 1 DEFAULT ('[]'),
             "languages" BLOB SUB_TYPE 1 DEFAULT ('["pt","es"]'),
             "limits" BLOB SUB_TYPE 1 DEFAULT ('[100,30000]'))
             """
             |> remove_newlines
           ]
  end
  ...
  test "create table with binary column" do
    create = {:create, table(:blobs), [{:add, :blob, :binary, [default: "foo"]}]}

    assert execute_ddl(create) == [
             """
             CREATE TABLE "blobs" ("blob" BLOB SUB_TYPE 0 DEFAULT 'foo')
             """
             |> remove_newlines
           ]
  end
  ...
  test "create table with binary column and hex blob literal default" do
    ...
    assert execute_ddl(create) == [
             """
             CREATE TABLE "blobs" ("blob" BLOB SUB_TYPE 0 DEFAULT '\\\\x666F6F')
             """
             |> remove_newlines
           ]
  end
  ...
  test "create table with binary column and hex blob literal null-byte" do
    ...
    assert execute_ddl(create) == [
             """
             CREATE TABLE "blobs" ("blob" BLOB SUB_TYPE 0 DEFAULT '\\\\\x00')
             """
             |> remove_newlines
           ]
  end
  ...
  test "create table with a map column, and an empty map default" do
    ...
    assert execute_ddl(create) == [
             ~s|CREATE TABLE "posts" ("a" BLOB SUB_TYPE 1 DEFAULT ('{}'))|
           ]
  end
  ...
  test "create table with a map column, and a map default with values" do
    ...
    assert execute_ddl(create) == [
             ~s|CREATE TABLE "posts" ("a" BLOB SUB_TYPE 1 DEFAULT ('{"baz":"boom","foo":"bar"}'))|
           ]
  end
  ...
  test "create table with a map column, and a string default" do
    ...
    assert execute_ddl(create) == [
             ~s|CREATE TABLE "posts" ("a" BLOB SUB_TYPE 1 DEFAULT '{"foo":"bar","baz":"boom"}')|
           ]
  end
  ...
  test "create table with time columns" do
    ...
    assert execute_ddl(create) == [
             ~s|CREATE TABLE "posts" ("published_at" TIME, "submitted_at" TIME)|
           ]
  end
  ...
  test "create table with utc_datetime columns" do
    ...
    assert execute_ddl(create) == [
             ~s|CREATE TABLE "posts" ("published_at" TIMESTAMP WITH TIMEZONE, "submitted_at" TIMESTAMP WITH TIMEZONE)|
           ]
  end
  ...
  test "create table with naive_datetime columns" do
    ...
    assert execute_ddl(create) == [
             ~s|CREATE TABLE "posts" ("published_at" TIMESTAMP, "submitted_at" TIMESTAMP)|
           ]
  end
  ...
  test "alter table" do
    ...
    assert execute_ddl(alter) == [
             ...
             ~s|ALTER TABLE "posts" ADD COLUMN "category_id" BIGINT CONSTRAINT "posts_category_id_fkey" REFERENCES "categories"("id")|,
  ...
  test "alter table with serial primary key" do
    ...
    assert execute_ddl(alter) == [
             """
             ALTER TABLE "posts"
             ADD COLUMN "my_pk" INTEGER GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY
             """
             |> remove_newlines
           ]
  end
  ...
  test "alter table with bigserial primary key" do
    ...
    assert execute_ddl(alter) == [
             """
             ALTER TABLE "posts"
             ADD COLUMN "my_pk" BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY
             """
             |> remove_newlines
           ]
  end
  ...
  test "drop constraint" do
    assert_raise ArgumentError,
                 ~r/Firebird does not support ALTER TABLE DROP CONSTRAINT./,
                 fn ->
                   execute_ddl(
                     {:drop,
  ...
  test "autoincrement support" do
    ...
    assert execute_ddl(serial) == [
             ~s/CREATE TABLE "posts" ("id" INTEGER GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY)/
           ]

    assert execute_ddl(bigserial) == [
             ~s/CREATE TABLE "posts" ("id" BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY)/
           ]
  ...
  #    alias Ecto.Adapters.Firebird.Connection
  #
