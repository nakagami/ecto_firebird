defmodule Ecto.Adapters.FirebirdTest do
  ...
  defp plan(query, operation \\ :all) do
    {query, _params} = Ecto.Adapter.Queryable.plan_query(operation, Ecto.Adapters.Firebird, query)
    query
  end
  ...
  test "aggregate filters" do
    ...
    assert_raise Ecto.QueryError,
                 ~r/Firebird adapter does not support aggregate filters in query/,
                 fn ->
                   all(query)
                 end
  end
  ...
  test "limit and offset" do
    ...
    assert all(query) ==
             ~s{SELECT TRUE FROM "schema" AS s0 FETCH FIRST 3 ROWS ONLY  OFFSET 5 ROWS }
  end
  ...
  test "insert" do
    ...
    assert_raise ArgumentError,
                 ":returning is not supported in insert/insert_all by Firebird",
                 fn ->
  ...
  test "insert with on duplicate key" do
    ...
    assert_raise ArgumentError,
                 "The :conflict_target option is not supported in insert/insert_all by Firebird",
                 fn ->
                   insert(nil, "schema", [:x, :y], [[:x, :y]], {[:x, :y], [], [:x]}, [])
                 end

    assert_raise ArgumentError,
                 "Using a query with :where in combination with the :on_conflict option is not supported by Firebird",
                 fn ->
                   update =
  ...
  test "alter table with invalid reference opts" do
    ...
    assert_raise ArgumentError,
                 "validate: false on references is not supported in Firebirdex",
                 fn ->
                   execute_ddl(alter)
                 end
  end
